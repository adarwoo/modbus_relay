...
#define ADC_SAMPLE_FREQUENCY 320 // Default sample frequency in Hz
   ...
   if ( fft.wInc > 0 )
   {
      ...
      if ( fft.i != (FFT_N - 1) )
      {
         ...
         if ( fft.bin & fft.gSize ) // Binary AND here
         {
            ...
            memcpy( &w, &fftTwiddle[fft.wIndex], sizeof(complex_t) );
      ...
      else // Last sample is different. No butterfly, but PSD calculation
      {
         ...
         // Compute the power
         fft.result = sqrt(
            (fft.x[FFT_H - 1].real) * (fft.x[FFT_H - 1].real) +
            (fft.x[FFT_H - 1].imag) * (fft.x[FFT_H - 1].imag) );
